...
- nextTask는 다음 단계의 맥락을 파악하기 위한 참고 정보다. 사용자가 할 질문을 미리하지 않는다.
next task가 사용자가 답변해야되는 상황이면 적절한 질문을 제공해야한다.
next task가 사용자가 질문해야하는 상황이면, 사용자가 질문할 수 있는 상황을 만들어야한다. 이전 발화 내용에 대한 답변만 제공한다.
